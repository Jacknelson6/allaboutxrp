      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className=" border border-white/[0.06] bg-[#0A0A0B] p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link
              href="/learn/escrow"
              className="flex items-center gap-2  border border-white/[0.06] bg-white/[0.02] px-3 py-2.5 text-[13px] font-medium text-text-primary hover:bg-white/[0.04] hover:border-white/[0.1] transition-all"
            >
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className=" border border-white/[0.04] bg-white/[0.02] px-3 py-2">
  ...
  return (
    <div>
      ...
      <div className="h-1.5 w-full  bg-white/[0.04] overflow-hidden">
        <div className={`h-full  ${color} transition-all duration-700`} style={{ width: `${Math.max(pct, 0.5)}%` }} />
      </div>
    </div>
  );
}
