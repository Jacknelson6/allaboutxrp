...
export async function generateBlogForArticle(id: string): Promise<{ success: boolean; error?: string }> {
  if (!OPENROUTER_KEY) return { success: false, error: "No API key" };
  ...
  if (artErr || !article) return { success: false, error: "Article not found" };
  if (article.blog_content) return { success: true };
  ...
  const internalPagesList = INTERNAL_PAGES.map((p) => `- ${p.path} — ${p.label}`).join("\n");
...
Where related_index is the [index] of the related article you referenced in paragraph 3.`;
  ...
  try {
    ...
    const blogContent = content.replace(/METADATA:[\s\S]*$/, "").trim();
