        ...
        <div className="cv-auto mt-10 space-y-14">
                  ...
                  <div className="mt-4 space-y-4 border-l-2 border-white/[0.06] pl-6">
                    {eventsByYear[year].map((event, i) => (
                      <div key={`${event.date}-${i}`} className="relative">
                        <div className="absolute -left-[31px] top-1 h-3 w-3  border-2 border-white/[0.06] bg-black" />
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-xs font-mono text-text-secondary">
                            {new Date(event.date + "T12:00:00Z").toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                          </span>
                          <span className={` border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${categoryColors[event.category] || "bg-[#111113] text-text-secondary border-white/[0.06]"}`}>
                            {event.category}
                          </span>
          ...
          <RevealSection delay={0.05}>
            ...
            <div className="mt-5 space-y-4">
              {[
                ...
                <div key={item.num} className="flex gap-4  border border-white/[0.06]/60 bg-black p-5 ">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center  bg-xrp-accent/10 font-mono text-sm font-bold text-xrp-accent">{item.num}</span>
                  <div>
          ...
          <RevealSection delay={0.05}>
            ...
            <p className="mt-3 text-text-secondary leading-relaxed">
              The <Link href="/learn/riddlers" className="text-xrp-accent underline decoration-xrp-accent/30">Riddler tradition</Link> involving figures like BearableGuy123 and Mr. Pool is unlike anything in other crypto communities. For research-oriented accounts and primary sources, use our <Link href="/learn/trusted-sources" className="text-xrp-accent underline decoration-xrp-accent/30">trusted XRP sources</Link> page.
            </p>
          </RevealSection>
